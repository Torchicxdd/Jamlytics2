...
const GRAZING_POINTS_INTERVAL: float = 0.10
const GRAZING_POINTS_PER_INTERVAL: int = 100
...
func _process(delta: float) -> void:
	for bullet in _bullets.keys():
		...
		if _bullets[bullet] >= GRAZING_POINTS_INTERVAL:
			_bullets[bullet] = 0.0
			GameManager.add_points(GameManager.Powerup.POISE, GRAZING_POINTS_PER_INTERVAL)
